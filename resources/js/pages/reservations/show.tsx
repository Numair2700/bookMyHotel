...
import { CalendarDays, CheckCircle2, CreditCard, Leaf, ShieldCheck, Star } from 'lucide-react';
...
function PaymentPanel({ reservation }: { reservation: ReservationDetail }) {
    const { data, setData, post, processing, errors } = useForm({ method: 'card', token: 'tok_visa_ok' });
    ...
    const field = 'w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20';
    ...
    return (
        <form onSubmit={submit} className="rounded-xl border border-border bg-card p-6">
            <h2 className="flex items-center gap-2 font-serif text-xl font-semibold text-foreground">
                <CreditCard className="h-5 w-5 text-primary" /> Pay {money(reservation.total_amount)}
            </h2>
            {errors.payment && (
                <p className="mt-3 rounded-lg bg-destructive/10 px-3 py-2 text-sm text-destructive">{errors.payment}</p>
            )}
...
function ReviewPanel({ reservationId }: { reservationId: number }) {
    const { data, setData, post, processing, errors } = useForm({ rating: 9, comment: '' });
    ...
    return (
        <form onSubmit={submit} className="mt-6 rounded-xl border border-border bg-card p-6">
            ...
            <div className="mt-4">
                <label className="mb-1 block text-sm font-medium text-foreground" htmlFor="rating">
...
export default function ReservationShow({ reservation, refund_quote, can_review, review }: Props) {
    ...
    const doCancel = () => {
        if (confirm('Cancel this reservation?')) {
            cancel.post(`/reservations/${reservation.id}/cancel`, { preserveScroll: true });
        }
    };
    ...
    return (
        <>
            ...
            <section className="relative isolate">
                <img src={hotelImage(reservation.hotel_id ?? reservation.id)} alt="" className="absolute inset-0 -z-10 h-full w-full object-cover" />
                ...
                <div className="mx-auto flex min-h-56 max-w-5xl flex-col justify-end px-4 py-8 sm:px-6">
                    <p className="text-sm text-white/80">Booking {reservation.reference}</p>
                    <h1 className="mt-1 font-serif text-3xl font-semibold text-white sm:text-4xl">{reservation.hotel}</h1>
                    <p className="mt-2 flex items-center gap-1.5 text-white/85">
                        <CalendarDays className="h-4 w-4" /> {reservation.check_in} → {reservation.check_out} ·{' '}
                    ...
                    <div className="mb-6 flex items-center gap-3 rounded-xl border border-eco/40 bg-eco/10 px-5 py-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                A confirmation has been sent to your email. Reference {reservation.reference}.
                            </p>
                        </div>
                    </div>
                )}
                    ...
                    <div className="lg:col-span-3">
                        <div className="rounded-xl border border-border bg-card p-6">
                            <div className="flex items-center justify-between">
                                <h2 className="font-serif text-xl font-semibold text-foreground">Stay summary</h2>
                                {reservation.is_sustainable && (
                                    <span className="inline-flex items-center gap-1 rounded-full bg-eco/15 px-2.5 py-1 text-xs font-medium text-eco">
                                        <Leaf className="h-3.5 w-3.5" /> Sustainable stay
                                    </span>
                                )}
                            </div>
                            <table className="mt-4 w-full text-sm">
                                <tbody>
                                    {reservation.nights.map((night) => (
                                        <tr key={night.stay_date} className="border-b border-border/60">
                                            <td className="py-2 text-muted-foreground">{night.stay_date}</td>
                                            <td className="py-2 text-right text-foreground">{money(night.rate)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            <dl className="mt-4 space-y-1.5 text-sm">
                                ...
                                {reservation.discount_total > 0 && (
                                    <div className="flex justify-between text-eco">
                                        <dt>Discount</dt>
                                        <dd>− {money(reservation.discount_total)}</dd>
                                    </div>
                                )}
                        ...
                        {refund_quote && (
                            <div className="mt-6 rounded-xl border border-border bg-card p-6">
                                <h2 className="flex items-center gap-2 font-serif text-xl font-semibold text-foreground">
                                    <ShieldCheck className="h-5 w-5 text-primary" /> Cancellation
                                </h2>
                                <p className="mt-2 text-sm text-muted-foreground">
                                    {refund_quote.policy_name ?? 'Standard policy'}. Cancel now and you would be refunded{' '}
                                    <span className="font-semibold text-foreground">{money(refund_quote.amount)}</span> (
                        ...
                        {review ? (
                            <div className="mt-6 rounded-xl border border-border bg-card p-6">
                                <h2 className="flex items-center gap-2 font-serif text-xl font-semibold text-foreground">
                                    <Star className="h-5 w-5 text-gold" /> Your review
                                </h2>
                                <p className="mt-2 text-sm font-medium text-foreground">{review.rating}/10</p>
                                {review.comment && (
                    ...
                    <div className="lg:col-span-2">
                        {reservation.status === 'pending' ? (
                            ...
                            <div className="rounded-xl border border-border bg-secondary/40 p-6 text-sm text-muted-foreground">
                                <p className="font-medium text-foreground capitalize">Status: {reservation.status}</p>
                                <p className="mt-1">No payment is due on this booking.</p>
                            </div>
                        )}
                    </div>
                </div>
