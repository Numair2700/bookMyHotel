...
export default function ManagerPromotions({ promotions, hotels }: Props) {
    ...
    const submit = (e: FormEvent) => {
        ...
        post('/manager/promotions', {
            preserveScroll: true,
            onSuccess: () => reset('code', 'description'),
        });
    };

    const withdraw = (id: number) => {
        if (confirm('Withdraw this promotion?')) {
            router.delete(`/manager/promotions/${id}`, {
                preserveScroll: true,
            });
        }
    };
    ...
    return (
        <>
            ...
            <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-3">
                ...
                <form
                    onSubmit={submit}
                    className="h-fit rounded-xl border border-border bg-card p-6 lg:sticky lg:top-24"
                >
                    ...
                    <div className="mt-4 space-y-3">
                        <div>
                            <label className={label} htmlFor="hotel_id">
                                Hotel
                            </label>
                            <select
                                id="hotel_id"
                                value={data.hotel_id}
                                onChange={(e) =>
                                    setData('hotel_id', Number(e.target.value))
                                }
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                <label className={label} htmlFor="valid_to">
                                    To
                                </label>
                                <input
                                    id="valid_to"
                    ...
                    <button
                        type="submit"
                        disabled={processing}
                        className="mt-4 w-full cursor-pointer rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground hover:opacity-90 disabled:opacity-60"
                    >
                        Create promotion
                    </button>
                </form>
