            ...
            <div className="border-b border-border bg-secondary/40">
                <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
                    <h1 className="font-serif text-2xl font-semibold text-foreground sm:text-3xl">
                        {criteria.destination ? `Stays in ${criteria.destination}` : 'Available stays'}
                    </h1>
                    <p className="mt-2 text-sm text-muted-foreground">
                        {criteria.check_in} → {criteria.check_out} · {criteria.nights}{' '}
            ...
            <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
                {results.length === 0 ? (
                    <div className="rounded-xl border border-dashed border-border bg-card p-12 text-center">
                        <p className="font-serif text-lg text-foreground">No rooms available for these dates.</p>
                        <p className="mt-2 text-sm text-muted-foreground">Try a different destination or date range.</p>
                        <Link
                            href="/"
                            ...
                            >
                                ...
                                <div className="flex flex-1 flex-col p-5">
                                    <div className="flex items-center gap-1 text-gold">
                                        {Array.from({ length: result.hotel.star_rating }).map((_, i) => (
                                    ...
                                    <div className="mt-4 rounded-lg bg-secondary/60 p-3">
                                        <p className="text-sm font-medium text-foreground">{result.room_type.name}</p>
                                        <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                                            <span className="inline-flex items-center gap-1">
                                                <Users className="h-3.5 w-3.5" /> Sleeps {result.room_type.max_occupancy}
                                            </span>
                                    ...
                                    <div className="mt-auto flex items-end justify-between pt-5">
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {money(result.total_price)} total · {result.nights}{' '}
